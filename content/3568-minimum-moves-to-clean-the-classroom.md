...
---
### Python3
``` py title='minimum-moves-to-clean-the-classroom'
class Solution:
    def minMoves(self, grid: List[str], startEnergy: int) -> int:
        ...
        litterIndex = {(x, y) : i for i, (x, y) in enumerate(litter)}
        litterCount = len(litter)

        if litterCount == 0: return 0

        visited = [[[-inf] * (1 << litterCount) for _ in range(cols)] for _ in range(rows)]
        visited[sx][sy][0] = startEnergy
        allMask = (1 << litterCount) - 1
        queue = deque()
        queue.append((sx, sy, startEnergy, 0, 0))
        
        while queue:
            x, y, energy, mask, moves = queue.popleft()
            ...
            for dx, dy in [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)]:
                if 0 <= dx < rows and 0 <= dy < cols and grid[dx][dy] != 'X':
                    newMask = mask
                    newEnergy = energy - 1
                    if grid[dx][dy] == 'R':
                        newEnergy = startEnergy 
                    elif grid[dx][dy] == 'L':
                        newMask |= (1 << litterIndex[(dx, dy)])
                        
                    if newEnergy >= 0 and newEnergy > visited[dx][dy][newMask]:
                        visited[dx][dy][newMask] = newEnergy
                        queue.append((dx, dy, newEnergy, newMask, moves + 1))
