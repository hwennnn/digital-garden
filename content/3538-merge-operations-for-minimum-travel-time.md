...
[Problem Link](https://leetcode.com/problems/merge-operations-for-minimum-travel-time/)
...
---
### Python3
``` py title='merge-operations-for-minimum-travel-time'
class Solution:
    def minTravelTime(self, l: int, n: int, k: int, position: List[int], time: List[int]) -> int:

        @cache
        def go(index, remK, prevTime):
            if index == n - 1: return 0 if remK == 0 else inf

            res = inf
            currTime = 0
            for j in range(index + 1, n):
                if remK - (j - index - 1) < 0: break
                    
                dist = position[j] - position[index]
                cost = dist * prevTime
                currTime += time[j]
                res = min(res, cost + go(j, remK - (j - index - 1), currTime))

            return res

        return go(0, k, time[0])  
```
