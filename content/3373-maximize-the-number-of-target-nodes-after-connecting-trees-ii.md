...
---
### Python3
``` py title='maximize-the-number-of-target-nodes-after-connecting-trees-ii'
class Solution:
        ...
        def build(edges):
            ...
            def dfs(node, prev, currColor):
                ...
                for adj in graph[node]:
                    if adj != prev:
                        dfs(adj, node, 1 - currColor)
            
            dfs(0, -1, 0)

            return evenCount, N - evenCount, color
        
        even1, odd1, color1 = build(edges1)
        even2, odd2, _ = build(edges2)
        d2 = max(even2, odd2)

        return [(even1 if color1[node] == 0 else odd1) + d2 for node in range(len(edges1) + 1)]
```
